...
def build_temporal_figures(
    ...
    """Genera y sincroniza las cuatro gráficas del mismo escenario."""
    ...
    formation_figure = build_formation_error_figure(
        result=result,
        scenario_name=normalized_scenario,
    )

    soc_figure = build_soc_figure(
        result=result,
        scenario_name=normalized_scenario,
    )

    link_margin_figure = build_link_margin_figure(
        result=result,
        scenario_name=normalized_scenario,
    )

    availability_figure = build_isl_availability_figure(
        result=result,
        scenario_name=normalized_scenario,
    )
    ...
    titles = (
        f"Error de formación — {normalized_scenario}",
        f"Estado de carga de batería — {normalized_scenario}",
        (
            "Margen del enlace ISL en banda Ka — "
            f"{normalized_scenario}"
        ),
        (
            "Disponibilidad del enlace intersatelital — "
            f"{normalized_scenario}"
        ),
    )
    ...
    # El escenario forma parte de uirevision. Al cambiarlo, Plotly
    # descarta el estado visual anterior en vez de conservar LOW_POWER.
    # datarevision cambia con el número de snapshots y fuerza la
    # actualización progresiva de los datos del escenario actual.
    revision_token = (
        f"{normalized_scenario}:"
        f"{snapshot_count}"
    )
    ...
    for figure, title in zip(
        ...
        figure.update_layout(
            title_text=title,
            uirevision=normalized_scenario,
